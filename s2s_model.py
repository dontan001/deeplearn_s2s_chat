    ...
    def __init__(self,
        ...
        def seq2seq_f(encoder_inputs, decoder_inputs, do_decode):
            ...
            return tf.contrib.legacy_seq2seq.embedding_attention_seq2seq(
                ...
                embedding_size=size,# embedding 维度， 用于表示一个字 ？？
...
# 跟language model类似，targets变量是decoder inputs平移一个单位的结果，
    ...
    def get_batch(self, bucket_dbs, bucket_id, data):
        ...
        for i in range(decoder_size):
            ...
            batch_weight = np.ones(self.batch_size, dtype=np.float32) # weight default to 1
            for j in range(self.batch_size):
                if i < decoder_size - 1:
                    target = decoder_inputs[j][i + 1] # 偏移一位
                if i == decoder_size - 1 or target == data_utils.PAD_ID:
                    batch_weight[j] = 0.0 # noise data to 0
